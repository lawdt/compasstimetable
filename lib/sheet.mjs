...
function parseSchedule(html, { sourceUrl } = {}) {
  ...
  headers.forEach((header, i) => {
    ...
    for (const id of Object.keys(byClass)) {
      ...
      let n = 0;
      // Обычную программу от активностей второй половины дня отделяет
      // служебный блок — обед, прогулка или свободная игра. Годится только
      // тот, после которого ещё есть занятия: самоподготовка встречается и
      // в самом конце дня, уже после платных, и границей быть не может.
      // Если такого блока нет, значит и платных занятий в этот день нет.
      const isBoundary = (e) => e.kind === 'free'
        || (e.kind === 'break' && LUNCH_RE.test(e.raw));
      const lastLesson = byClass[id]
        .reduce((at, e, i) => (e.kind === 'lesson' ? i : at), -1);
      let boundary = -1;
      byClass[id].forEach((e, i) => { if (isBoundary(e) && i < lastLesson) boundary = i; });

      byClass[id].forEach((e, i) => {
        delete e.row;
        e.index = e.kind === 'lesson' ? (n += 1) : 0;
        // boundary < 0 означает, что разделителя нет вовсе: в такой день
        // платных занятий тоже нет.
        e.afternoon = e.kind === 'lesson' && boundary >= 0 && i > boundary;
